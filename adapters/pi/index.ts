...
/**
 * Run the fiefdom CLI — the same binary the Claude Code adapter drives — and
 * return its combined output. Setup and review are done this way so Pi shares
 * one implementation with Claude Code instead of keeping its own drifting copy.
 */
async function runFiefdom(
	subcommand: string,
	cwd: string
): Promise<{ ok: boolean; output: string }> {
	try {
		const { stdout, stderr } = await execFileAsync("sh", ["-c", `${resolveBin()} ${subcommand}`], {
			cwd,
...
interface FiefdomState {
	...
	childFief: { config: FiefdomConfig; fief: FiefConfig } | null;
}
...
export default function fiefdom(pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (process.env.PI_FIEFDOM_CHILD === "1") {
			...
			state.childFief = childConfig && fief ? { config: childConfig, fief } : null;
			return;
		}
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		// Fief worker: may only write inside its own territory. The judgement is
		// the shared one core/enforce.ts also gives the Claude Code hook, so both
		// harnesses enforce the boundary identically.
		if (state.childFief) {
			if (!targetPath) return;

			const { config, fief } = state.childFief;
			const relative = toRepoRelative(targetPath, config.paths.repoRoot, ctx.cwd);
			if (relative === null) return; // Outside the repo: not fiefdom's business.

			// ponytail: only file writes are judged here. A worker's shell writes
			// (sed -i, heredocs) slip past — the Claude hook catches those via
			// core/bashwrites; wiring the same into Pi needs its shell-tool event
			// shape, which is not available here.
			const decision = decideFiefWrite(config, fief, [{ relative, reason: event.toolName }]);
	...
	pi.registerCommand("fiefdom-setup", {
		...
		handler: async (_args, ctx) => {
			if (!fs.existsSync(path.join(ctx.cwd, ".git"))) {
				ctx.ui.notify("Fiefdom requires a git repository.", "error");
				return;
			}

			// Re-running over an existing config only re-syncs; --force re-analyses
			// from scratch, so confirm before throwing the current division away.
			let force = false;
			if (state.initialized) {
				force = await ctx.ui.confirm(
					"Fiefdom already configured",
					"Re-analyse and regenerate the fief division from scratch? Existing memory and hand-written personas are kept."
				);
			}

			ctx.ui.notify("Analyzing repository and writing config...", "info");

			// The Claude Code adapter's `init` is the one implementation of setup;
			// it analyses, writes .fiefdom/, and syncs the Claude side too, so the
			// repo then opens under either harness.
			const { ok, output } = await runFiefdom(force ? "init --force" : "init", ctx.cwd);
			ctx.ui.notify(output || (ok ? "Configured." : "Setup failed."), ok ? "info" : "error");
			if (ok) ctx.ui.notify("Restart the session or /reload to activate.", "info");
		},
	});
	...
	pi.registerCommand("fiefdom-review", {
		...
		handler: async (_args, ctx) => {
			if (!state.initialized || !state.config) {
				ctx.ui.notify("Fiefdom not configured. Use /fiefdom-setup first.", "error");
				return;
			}

			// Same report the Claude Code adapter produces (core coverage/co-change
			// analysis), shown in an editor.
			const { output } = await runFiefdom("review", ctx.cwd);
			await ctx.ui.editor("Fiefdom Review", output || "(no output)");
		},
	});
}
